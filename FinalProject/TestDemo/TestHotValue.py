...
log_file_handler = TimedRotatingFileHandler(filename="analysis/hvg.log",
                                            when="S", interval=5,
                                            backupCount=20)
    ...
    def calHotValue(self):
        ...
        for row in result_list:
            try:
                ...
                # hot_value = (math.log(math.e,1+row[2])+1.75*row[3])/math.exp(diff.days+1)*10000000
                hot_value = ((math.log(1+row[2])+1.75*row[3])/ math.exp((diff.days+1)*0.05))*1000
                print(hot_value)
                logger.info("HotValue:{}".format(hot_value))
                result.append((row[0], row[1], format(hot_value, ".3f")))
            except Exception:
                logger.error("转换出错")
