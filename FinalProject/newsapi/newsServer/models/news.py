...
#浏览新闻时，若用户标签出现在文章关键词中则+0.01，若没出现，设为0.01的用户标签
        ...
        if int(userid) != 100000:
            ...
            for keyword in newskeywords:
                if keyword in weight:
                    weight[keyword] = float(format(weight[keyword] + 0.1, ".3f"))
                    if weight[keyword] >= 0.5:
