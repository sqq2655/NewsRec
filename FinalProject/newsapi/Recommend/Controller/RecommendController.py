...
def beginRecommendSystem(time):
    '''
    推荐系统启动管理器（基于热度推荐、基于新闻标签推荐）
    '''
    sched.add_job(beginrecommendbyhotvalue, 'interval', max_instances=1, seconds=int(time),
                  id='NewsRecommendByHotValue',
                  kwargs={})
    sched.add_job(beginNewsRecommendByTags, 'interval', max_instances=1, seconds=int(time),
                  id='NewsRecommendByTags',
                  kwargs={})
    sched.start()
...
def beginAnalysisSystem(time):
    '''
        数据分析系统启动管理器（关键词分析、热词分析、新闻相似度分析、热词统计）
    '''
