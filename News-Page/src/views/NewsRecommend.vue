      ...
      <Row>
        ...
        <Col span="12" style="height: auto; padding: 10px; cursor: pointer;">
          <Card :bordered="false" class="newsitem" v-for="(item, index) in this.newslist.slice(0,a)" :key="'news'+index"
            ...
            <Row>
              <Col :lg="7" style="color: #a8a8a8" ><Icon size="20" type="ios-people" />{{item.readnum}} &nbsp;&nbsp; <Icon size="18" type="ios-chatbubbles" />{{item.comments}}</Col>
              <Col :lg="10">
                <Tag v-for="kword in item.keywordslist" :key="kword">{{kword}}</Tag>
              </Col>
              <Col :lg="7" style="text-align: right;">推荐By：{{ item.species }}</Col>
            </Row>
          </Card>
          <div align="center">
  ...
  created() {
    ...
    getRecNewsDetail(sessionStorage.getItem('userId')).then(res => {
      ...
      for (let i = 0; i < newslist.length; i++) {
        ...
        let keywordslist = newslist[i].keywords.split(',')
        if (keywordslist.length > 4)
          keywordslist=keywordslist.slice(0,4)
          keywordslist.push('...')
        switch (species){
          case 0:
            species = '兴趣'
