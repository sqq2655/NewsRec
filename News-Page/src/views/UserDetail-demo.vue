            ...
            <Row type="flex">
              <Col :xxs="1" :xs="5" :md="10" :lg="18" >
                <Card >
                  ...
                  <el-upload
                    name="image"
                    :data="{apiType:'ali', token:'89b721d4a197df8e20607ea1e5c92b66'}"
...
export default {
  ...
  methods: {
    handleAvatarSuccess(res, file) {
      ...
      console.log(this.userdetail.headportrait);
    ...
    deleteUsertags(param) { //删除用户自己的标签
      let maskImage = new Image();
    ...
    addTagok() { //用户自定义添加
      if(this.newtag !== ''){
        ...
        let tag_temp = this.newtag.split(',')
        for(let temp in tag_temp){
          this.myTags.push({name:tag_temp[temp], value:Math.random()*30})
          tags.push(tag_temp[temp])
        }
